...
const loadAllPetsCategoriesFromApi = async() =>{
 ...
 async function ActiveButton(buttonName){
     ...
     for(const oneButton of allButton){
         ...
         oneButton.classList.remove('bg-sky-200', 'bg-opacity-10' ,'rounded-full','border-[#0E7A811A]')
     }
 
     // adding active button features to clicked button
     document.getElementById(buttonName).classList.add('bg-sky-200', 'bg-opacity-10' ,'rounded-full','border-[#0E7A811A]')
...
//  T A S K   8 :  SORTING IN DESCENDING ORDER

async function sort(){

variable.sort(function(a, b){return b.price- a.price })
displayAndMakeCardOfAllPetsFromApi(variable)
}